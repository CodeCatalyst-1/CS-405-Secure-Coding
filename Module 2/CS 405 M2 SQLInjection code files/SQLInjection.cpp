...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // This implementation specifically detects "OR value=value" style SQL injection attacks
    // where value can be numeric (e.g., 1=1, 2=2) or quoted strings (e.g., 'a'='a').

    // Clear any prior results
    records.clear();

    // Create a lowercase copy of the SQL string for case-insensitive comparison
    std::string sql_lower(sql);
    std::transform(sql_lower.begin(), sql_lower.end(), sql_lower.begin(),
        [](unsigned char c) { return std::tolower(c); });

    // Regular expression to detect:
    //  - "or <number>=<number>"
    //  - "or '<text>'='<text>'"
    // This is generic and does NOT rely on detecting a specific value like 1=1
    std::regex injection_pattern(
        R"(\bor\s+(\d+)\s*=\s*\1|\bor\s+'([^']+)'\s*=\s*'\2')",
        std::regex_constants::icase
    );

    // If the SQL matches the injection pattern, block execution
    if (std::regex_search(sql_lower, injection_pattern))
    {
        std::cout << "WARNING: Possible SQL Injection attempt detected." << std::endl;
        std::cout << "Blocked SQL: " << sql << std::endl << std::endl;
        return false;
    }

    // Execute query only if it passed validation
